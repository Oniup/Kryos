...
void print_msg_with_conditional_msg_v(b8 condition, FILE* p_out, const char* p_fmt, va_list args);
void print_msg_fmt(FILE* p_out, debug_log_level_flag_t lv, b8 ansi_col, const char* p_fpath,
                   i32 line, const char* p_expr, const char* p_fmt, va_list args);
...
void intl_print_log_msg(debug_log_level_flag_t lv, const char* p_fpath, i32 line,
    ...
    va_list args;
    va_start(args, p_fmt);
    print_log_msg_to_target_v(DEBUG_OUT_TARGET_LOG_FILE, lv, p_fpath, line, p_expr, p_fmt, args);
    if (lv > DEBUG_LOG_LEVEL_FLAG_WARN) {
        print_log_msg_to_target_v(DEBUG_OUT_TARGET_STDERR, lv, p_fpath, line, p_expr, p_fmt, args);
    } else {
        print_log_msg_to_target_v(DEBUG_OUT_TARGET_STDOUT, lv, p_fpath, line, p_expr, p_fmt, args);
    }
    va_end(args);
}
...
void print_log_msg_to_target_v(debug_out_target_t out, debug_log_level_flag_t lv,
    ...
    switch (out) {
        case DEBUG_OUT_TARGET_STDOUT:
            print_msg_fmt(stdout, lv, g_db_opts.enable.ansi_col, p_fpath, line, p_expr, p_fmt,
                          args);
            break;
        case DEBUG_OUT_TARGET_STDERR:
            print_msg_fmt(stderr, lv, g_db_opts.enable.ansi_col, p_fpath, line, p_expr, p_fmt,
                          args);
            break;
        case DEBUG_OUT_TARGET_LOG_FILE:
            {
                ...
                if (err != 0) {
                    ...
                    print_msg_fmt(p_stream, lv, g_db_opts.io.ansi_col, p_fpath, line, p_expr, p_fmt,
                                  args);
                    fclose(p_stream);
                }
            }
...
void print_msg_fmt(FILE* p_out, debug_log_level_flag_t lv, b8 ansi_col, const char* p_fpath,
    ...
    print_msg_with_conditional(g_db_opts.enable.file && p_fpath != NULL, p_out, "File: %s, ",
                               p_fpath);
    print_msg_with_conditional(g_db_opts.enable.line && line != -1, p_out, "Line: %d, ", line);
    print_msg_with_conditional(g_db_opts.enable.expr && p_expr != NULL, p_out, "expr: %s, ",
                               p_expr);
    if (p_fmt != NULL) {
        fprintf(p_out, "Msg: ");
