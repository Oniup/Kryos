...
allocated_memory_result_t create_dynamic_allocation(usize size) {
    _kint_allocator_header_t* p_header = malloc(sizeof(_kint_allocator_header_t) + size);
    if (p_header == NULL) {
...
_kint_resize_dynamic_allocation_capacity(_kint_allocator_header_t* p_header, void* p_data,
                                         usize size) {
    _kint_allocator_header_t* p_tmp = realloc(p_header, sizeof(_kint_allocator_header_t) + size);
    if (p_tmp == NULL) {
        return (allocated_memory_result_t) {
            .error_message =
